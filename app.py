...
def normalize_text(s):
    ...
    s = s.lower()
    s = re.sub(r"[^a-z0-9 ]+", " ", s)
    stop = {"de","da","do","das","dos","para","com","e","a","o","as","os","em",
            "por","kit","un","unidade","unidades","novo","original","produto"}
...
def analyze_offer(image):
    prompt = """
Analise este print de uma oferta do Mercado Livre e retorne SOMENTE JSON válido:
{
  "product_title_raw": "título visível",
  "product_title_clean": "título curto e limpo, mantendo marca/modelo importantes",
  "price_from": 343.30 ou null,
  "price_to": 277.10,
  "payment_type": "pix" ou "installments" ou "none",
  "installments": 6 ou null,
  "coupon": "CODIGO" ou null
}
...
def generate_headline(product):
    ...
    return re.sub(r"\s+", " ", response.output_text.strip().strip('"').strip("'")).upper()
...
def build_message(d, link, headline):
    blocks = [headline.strip(), d["product_title_clean"].strip(), price_line(d)]
...
def best_match_for_day(product, day):
    conn = get_db()
    rows = conn.execute(
        "SELECT product_clean, price_to, source FROM promos WHERE promo_date=?",
        (day,)
    ).fetchall()
    matches = []
    for r in rows:
        ...
        if score >= 0.72:
            matches.append((r["product_clean"], r["price_to"], r["source"], score))
...
def save_promo(d, link, headline, source="meu", promo_date=None):
    ...
    conn.execute("""
        INSERT INTO promos (
          promo_date, product_raw, product_clean, normalized,
          price_from, price_to, payment_type, installments,
          coupon, affiliate_link, headline, source, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        promo_date, d.get("product_title_raw"), d["product_title_clean"],
        normalize_text(d["product_title_clean"]),
        money_int(d.get("price_from")), money_int(d.get("price_to")),
        d.get("payment_type"), d.get("installments"),
        d.get("coupon"), link, headline, source,
...
def stats():
    ...
    mine = conn.execute(
        "SELECT COUNT(*) c FROM promos WHERE promo_date=? AND source='meu'",
        (today,)
    ).fetchone()["c"]
    return {"mine": mine, "goal": META_DIARIA, "remaining": max(0, META_DIARIA - mine)}
...
def api_analyze():
    ...
    if not image or not link:
        return jsonify({"ok": False, "error": "Envie o print e o link de afiliado."}), 400
    try:
        d = analyze_offer(image)
        if not d.get("product_title_clean") or d.get("price_to") is None:
            raise ValueError("Não consegui identificar produto/preço no print.")

        today_match = best_match_for_day(d["product_title_clean"], date.today().isoformat())
        yesterday_match = best_match_for_day(
            d["product_title_clean"], (date.today() - timedelta(days=1)).isoformat()
        )

        blocked_reason = None
        if today_match:
            blocked_reason = f"Já apareceu hoje: {today_match[0]} (R$ {today_match[1]})."
        elif yesterday_match and money_int(d["price_to"]) > int(yesterday_match[1]):
            blocked_reason = (
                f"Preço pior que ontem: ontem R$ {yesterday_match[1]} e hoje "
                f"R$ {money_int(d['price_to'])}."
            )
        ...
        return jsonify({
            ...
            "yesterday": None if not yesterday_match else {
...
def api_generate():
    ...
    if today_match:
        return jsonify({"ok": False, "blocked": True, "error": "Produto já apareceu hoje."}), 409
    if yesterday_match and money_int(d["price_to"]) > int(yesterday_match[1]):
        return jsonify({"ok": False, "blocked": True, "error": "Preço atual está pior que ontem."}), 409

    try:
        headline = generate_headline(d["product_title_clean"])
        message = build_message(d, link, headline)
        return jsonify({"ok": True, "headline": headline, "message": message})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def api_import_group():
    ...
    target = date.today() if which == "today" else date.today() - timedelta(days=1)
    if not text:
        return jsonify({"ok": False, "error": "Cole as mensagens do grupo."}), 400

    prompt = f"""
Extraia as promoções das mensagens abaixo e retorne APENAS JSON válido:
[
  {{
...
"""
    try:
        response = ai().responses.create(model=MODEL, input=prompt)
        promos = extract_json_array(response.output_text)
        imported = 0
        for p in promos:
            if not p.get("product_title_clean") or p.get("price_to") is None:
                continue
            d = {
            ...
            save_promo(d, "", "", source="outro_curador", promo_date=target.isoformat())
            imported += 1
...
def api_history():
    conn = get_db()
    rows = conn.execute("""
        SELECT promo_date, product_clean, price_to, source, headline, affiliate_link
        FROM promos ORDER BY id DESC LIMIT 150
    """).fetchall()
    return jsonify({"ok": True, "items": [dict(r) for r in rows], "stats": stats()})
